 ...
 */
        ...
        import com.superiorinfotech.publicbuddy.adapters.FoldersListViewAdapter;
        import com.superiorinfotech.publicbuddy.tasks.AsyncCopyMoveTask;
        import com.superiorinfotech.publicbuddy.tasks.AsyncDeleteTask;

public class FilesFoldersDialogFragment extends DialogFragment {

    //Context.
    private Context mContext;
    @SuppressWarnings("unused")
    private FilesFoldersDialogFragment mFilesFoldersFragment;
    private IncidentReporter mApp;
    ...
    public static FilesFoldersDialogFragment newInstance(int fileType){

        FilesFoldersDialogFragment dialogFragment = new FilesFoldersDialogFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mApp = (IncidentReporter) mContext;
        mFolderStateMap = new HashMap<String, Parcelable>();

        //Set the hidden files flag.
        SHOW_HIDDEN_FILES = false;//mApp.getSharedPreferences().getBoolean("SHOW_HIDDEN_FILES", false);

        listView = (ListView) rootView.findViewById(R.id.folders_list_view);
        //Fast scroll seems to trigger android bug: https://code.google.com/p/android/issues/detail?id=75516
//        listView.setFastScrollEnabled(true);
        listView.setVisibility(View.INVISIBLE);

        //Set the background color based on the theme.
        rootView.setBackgroundColor(Color.parseColor("#e5e4de"));

        rootDir = Environment.getExternalStorageDirectory().getPath();//mApp.getSharedPreferences().getString("DEFAULT_FOLDER", Environment.getExternalStorageDirectory().getPath());
        currentDir = rootDir;
...
//        ((CreateIncident) getActivity()).showFolderFragmentActionItems(dirPath,
//                getActivity().getMenuInflater(),
//                ((HomeActivity) getActivity()).getMenu(),
//                mIsPasteShown);
        getDialog().setTitle(dirPath);
...
//                                getFileExtension(fileName).equalsIgnoreCase("mkv") ||
        ...
        listView.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View view, int index, long arg3) {
                ...
                if (fileFolderTypeList.get(index)==FOLDER) {
                    ...
                    attachFile(fileFolderTypeList.get(index), fileIndex, currentDir, newPath);
                }
    ...
    public boolean getParentDir() {
        ...
        FilesFoldersDialogFragment.currentDir = parentFolder;
     ...
     *                 or a folder path.
     * @param index The index of the first song to attachFile. Pass 0 if itemType
     *              is FOLDER.
     * @param folderPath The path of the folder that should be played.
     */
    public void attachFile(int itemType, int index, String folderPath, String filePath) {
        Bundle args = getArguments();
        if(args!=null && args.getInt("fileType", -1)==itemType) {
            Intent data = getActivity().getIntent();
            data.putExtra("path", filePath);
            getTargetFragment().onActivityResult(getTargetRequestCode(), Activity.RESULT_OK, data);
            dismiss();
        }else{
            Toast.makeText(getActivity(), "This file is of wrong type", Toast.LENGTH_LONG).show();
        }
    }
...
//        mIsPasteShown =  true;
//        ((HomeActivity) getActivity()).showFolderFragmentActionItems(currentDir,
//                getActivity().getMenuInflater(),
//                ((HomeActivity) getActivity()).getMenu(),
//                true);
    ...
    public void setCurrentDir(String currentDir) {
        FilesFoldersDialogFragment.currentDir = currentDir;
    }
